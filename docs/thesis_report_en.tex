...
The gold dataset is a manually maintained, checked-in artifact rather than a live dataset synchronized automatically with the latest repository release. Each case is reviewed against the pinned source snapshot, and the dataset stores the wheel and source digests together with the review type. A benchmark run is accepted only when these identities and the index provenance match; otherwise the run is rejected or treated as draft evidence. This makes the current dataset auditable, but the manual maintenance process remains a limitation and does not replace independent human annotation.
...
The requested multi-repository evaluation is not yet complete. Table~\ref{tab:evaluation_coverage} separates completed evidence from planned experiments.
