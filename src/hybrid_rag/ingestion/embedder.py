"""
Embedder: converts NodeData → text chunk → embedding vector via Ollama.

Model: nomic-embed-text (768-dim). Called via Ollama HTTP API.
Chunks are created from node properties — one chunk per KG node.
"""
...
class Embedder:
    """Embeds NodeData objects using Ollama nomic-embed-text."""
    ...
    def embed_nodes(self, nodes: list[NodeData]) -> list[dict[str, Any]]:
        """
        Embed each node. Returns list of chunk dicts ready for VectorClient.upsert().
