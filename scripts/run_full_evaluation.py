...
def compute_ground_truth(graph: FalkorDBStore, case: QueryCase) -> set[str]:
    """Execute ground truth Cypher and return expected names."""
    try:
        raw = graph.query(case.ground_truth_cypher)
        names = set()
...
def evaluate_hit_rate(
    ...
    results = []
    for case in cases:
        gt_names = compute_ground_truth(graph, case)
        if not gt_names:
        ...
        logger.info(
            ...
            case.id, case.hops, hit, latency_ms, len(gt_names), len(retrieved_names),
        )
...
def evaluate_vector_only(
    ...
    for case in cases:
        ...
        if not gt_names:
            results.append({
                "id": case.id, "hops": case.hops, "hit": False, "gt_count": 0,
            })
            continue
        ...
        try:
            ...
            results.append({
                "id": case.id, "hops": case.hops, "hit": False, "gt_count": len(gt_names),
                "error": str(exc),
            })
            continue
        latency_ms = (time.perf_counter() - t0) * 1000
        ...
        retrieved_names = []
        for r in vector_results[:hit_at_k]:
            name = r.get("name", "") or r.get("payload", {}).get("name", "")
            if name:
                retrieved_names.append(name.lower())

        hit = bool(gt_names & set(retrieved_names))
        results.append({
            "id": case.id, "hops": case.hops, "query_type": case.query_type,
            "hit": hit, "gt_count": len(gt_names),
            "retrieved_names": retrieved_names, "latency_ms": round(latency_ms, 2),
        })
...
def compute_hit_rate(results: list[dict], label: str = "") -> dict:
    ...
    all_hits = [r["hit"] for r in results]
    summary = {
        "label": label,
        "overall": {
...
def generate_report(
    ...
    for h, v in zip(hybrid_results, vector_results):
        ...
        lines.append(f"| {h['id']} | {h['hops']} | {h.get('query_type', '')} | {v_hit} | {h_hit} | {lat:.0f} |")
    ...
    if latency_data:
        ...
        if all_lats:
            lines.append(f"| **Overall** | **{sum(all_lats)/len(all_lats):.0f}** | **{min(all_lats):.0f}** | **{max(all_lats):.0f}** |")
...
def main() -> None:
    ...
    parser.add_argument("--skip-vector-baseline", action="store_true", help="Skip vector-only baseline")
    parser.add_argument(
        "--falkordb-host", default=os.environ.get("FALKORDB_HOST", "localhost"),
    )
    parser.add_argument(
        "--falkordb-port", type=int, default=int(os.environ.get("FALKORDB_PORT", "6379")),
    )
    parser.add_argument(
    ...
    graph = FalkorDBStore(host=args.falkordb_host, port=args.falkordb_port)

    logger.info("Connecting to Qdrant at %s:%d ...", args.qdrant_host, args.qdrant_port)
    vector = QdrantStore(host=args.qdrant_host, port=args.qdrant_port)
    ...
    hybrid_results = evaluate_hit_rate(
        retriever, graph, EVAL_CORPUS, top_k=args.top_k, hit_at_k=args.hit_at_k,
    )
    ...
    if not args.skip_vector_baseline:
        ...
        vector_results = evaluate_vector_only(
            vector, graph, EVAL_CORPUS, top_k=args.top_k, hit_at_k=args.hit_at_k,
    ...
    report_md = generate_report(
        hybrid_results, vector_results, hybrid_results, ragas_data, timestamp,
    )
    ...
    print("=" * 60)
    print(f"Hybrid-RAG  Hit Rate @{args.hit_at_k}: {h_summary['overall']['hit_rate']:.3f} ({h_summary['overall']['hits']}/{h_summary['overall']['total']})")
    print(f"Vector-only Hit Rate @{args.hit_at_k}: {v_summary['overall']['hit_rate']:.3f} ({v_summary['overall']['hits']}/{v_summary['overall']['total']})")
    print(f"\nResults: {json_path}")
