...
def test_community_builder_partition_and_writeback(active_graph_store, monkeypatch):
    ...
    db_nodes = [
        ...
        NodeData(label="Class", id="app.db.Database", properties={"name": "Database", "file_path": "app/db.py"}),
        NodeData(label="Function", id="app.db.Database.connect", properties={"name": "connect", "file_path": "app/db.py"})
    ]
    db_edges = [
    ...
    ui_nodes = [
        ...
        NodeData(label="Class", id="app.ui.Router", properties={"name": "Router", "file_path": "app/ui.py"}),
        NodeData(label="Function", id="app.ui.Router.render", properties={"name": "render", "file_path": "app/ui.py"})
    ]
    ui_edges = [
    ...
    result = ParseResult(
        nodes=db_nodes + ui_nodes,
        edges=db_edges + ui_edges + bridge_edge
    )
    ...
    def mock_generate_report(self, comm_id, prompt):
        if comm_id.endswith("_0"):
            return "Database Layer", "This community manages connections and raw read/write database operations."
        else:
            return "UI Routing System", "This community handles routing, rendering, and web requests."
    ...
    comm_nodes_res = active_graph_store.query("MATCH (c:Community) RETURN c.id, c.name, c.summary ORDER BY c.id")
    ...
    deps_res = active_graph_store.query("MATCH (a:Community)-[r:COMMUNITY_DEPENDS]->(b:Community) RETURN r.weight")
