...
def test_resolve_global_links_dependent_repo_to_core_repo(active_graph_store):
    ...
    repo_a_nodes = [
        NodeData(
            label="Module",
            id="core_lib.math",
            properties={"name": "math", "file_path": "src/core_lib/math.py", "repository": "core-lib"}
        ),
        NodeData(
            label="Class",
            id="core_lib.math.Calculator",
            properties={"name": "Calculator", "file_path": "src/core_lib/math.py", "repository": "core-lib"}
        )
    ]
    repo_a_edges = [
    ...
    repo_b_nodes = [
        NodeData(
            label="Module",
            id="main_app.entry",
            properties={"name": "entry", "file_path": "src/main_app/entry.py", "repository": "main-app"}
        ),
        # External Stub node representing the inherited class
        NodeData(
            label="Module",
            id="Calculator",
            properties={"name": "Calculator", "file_path": "", "type": "external", "repository": "main-app"}
        ),
        NodeData(
            label="Class",
            id="main_app.entry.SuperCalculator",
            properties={"name": "SuperCalculator", "file_path": "src/main_app/entry.py", "repository": "main-app"}
        )
    ]
...
def test_qdrant_vector_scoping_payload_filters(active_vector_store):
    ...
    results_core = active_vector_store.search(
        embedding=[0.1] * 768,
        top_k=10,
        filter_payload={"repository": "core-lib"}
    )
    ...
    results_app = active_vector_store.search(
        embedding=[0.1] * 768,
        top_k=10,
        filter_payload={"repository": "main-app"}
    )
