...
class TestOllamaLLMExtractor:
    def test_returns_empty_when_no_local_nodes(self):
        ...
        result = ParseResult(
            nodes=[NodeData(label="Module", id="requests",
    ...
    def test_valid_response_produces_edge(self):
        ...
        result.nodes.append(NodeData(
            label="Class", id="b.py::SomeClass",
            properties={"name": "SomeClass", "file_path": "b.py"},
        ))

        llm_edges = [{"src_id": "a.py::MyClass::my_method", "rel": "USES",
                      "dst_name": "SomeClass", "confidence": 0.9}]
    ...
    def test_low_confidence_edge_filtered(self):
        ...
        llm_edges = [{"src_id": "a.py::foo", "rel": "USES",
                      "dst_name": "SomeClass", "confidence": 0.5}]  # below 0.7
    ...
    def test_generic_type_annotation_skipped(self):
        ...
        result = _parse_result("a.py::foo")
        llm_edges = [{"src_id": "a.py::foo", "rel": "USES",
                      "dst_name": "List[Node]", "confidence": 0.95}]
        with patch.object(extractor._client, "post", return_value=_mock_response(llm_edges)):
            edges = extractor.extract("code", result)
        assert edges == []
    ...
    def test_unknown_src_id_skipped(self):
        ...
        llm_edges = [{"src_id": "nonexistent::fn", "rel": "USES",
                      "dst_name": "SomeClass", "confidence": 0.9}]
        with patch.object(extractor._client, "post", return_value=_mock_response(llm_edges)):
            edges = extractor.extract("code", result)
        assert edges == []
    ...
    def test_duplicate_edge_not_added_if_already_in_result(self):
        extractor = _extractor()
        result = _parse_result("a.py::foo")
        result.nodes.append(NodeData(label="Class", id="b.py::Bar",
                                     properties={"name": "Bar", "file_path": "b.py"}))
        # Pre-existing edge
        result.edges.append(
            EdgeData(src_id="a.py::foo", rel="USES", dst_id="b.py::Bar")
        )
        llm_edges = [{"src_id": "a.py::foo", "rel": "USES",
                      "dst_name": "Bar", "confidence": 0.9}]
        with patch.object(extractor._client, "post", return_value=_mock_response(llm_edges)):
            edges = extractor.extract("code", result)
        assert edges == []
    ...
    def test_unknown_dst_becomes_stub_id(self):
        ...
        result = _parse_result("a.py::foo")
        llm_edges = [{"src_id": "a.py::foo", "rel": "USES",
                      "dst_name": "CrossFileClass", "confidence": 0.9}]
        with patch.object(extractor._client, "post", return_value=_mock_response(llm_edges)):
            edges = extractor.extract("code", result)
        assert len(edges) == 1
