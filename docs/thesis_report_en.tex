% Compile with XeLaTeX so the university template can use system fonts.
...
\usepackage{geometry}
\geometry{left=1.25in,right=0.75in,top=1in,bottom=1in,headheight=15pt}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{booktabs}
\usepackage{graphicx}
...
% The reference template uses approximately 20.7 pt leading for 12 pt body text.
\setstretch{1.43}
\setlength{\emergencystretch}{2em}

% Baseline heading hierarchy: one-line Arial chapter titles and Arial sections.
\titleformat{\chapter}[hang]
    {\normalfont\sffamily\fontsize{18}{21.6}\selectfont}
    {\thechapter}{0.6em}{}
\titlespacing*{\chapter}{0pt}{-14pt}{2pt}
\titleformat{name=\chapter,numberless}[block]
    {\normalfont\sffamily\fontsize{16}{19.2}\selectfont}
    {}{0pt}{\hspace*{-18pt}\makebox[\textwidth][c]}
\titlespacing*{name=\chapter,numberless}{0pt}{-26pt}{18pt}
\titleformat{\section}[hang]
    {\normalfont\sffamily\fontsize{16}{19.2}\selectfont}
    {\thesection}{0.6em}{}
\titlespacing*{\section}{0pt}{18pt}{10pt}
\titleformat{\subsection}[hang]
    {\normalfont\sffamily\bfseries\fontsize{14}{16.8}\selectfont}
    {\thesubsection}{0.6em}{}
...
% Main-matter pages show the page number at the upper-right, including chapter openings.
...
\newcommand{\thesistitletext}{RESEARCH AND DEVELOPMENT OF A CODE RETRIEVAL SYSTEM BASED ON HYBRID-RAG ARCHITECTURE COMBINING KNOWLEDGE GRAPHS AND SEMANTIC SEARCH}
\newcommand{\thesistitledisplay}{Research and Development of a Code Retrieval System Based on Hybrid-RAG Architecture Combining Knowledge Graphs and Semantic Search}
\newenvironment{physicalcenter}{%
    \par\noindent\hspace*{-18pt}\begin{minipage}[t]{\textwidth}\centering
}{%
    \end{minipage}\par
}
\newcommand{\frontmatterheading}[1]{%
    \begin{physicalcenter}
        {\sffamily\fontsize{16}{19.2}\selectfont #1\par}
    \end{physicalcenter}
    \vspace{12pt}
}
\newcommand{\frontmatterheadingleft}[1]{%
    {\raggedright\sffamily\fontsize{16}{19.2}\selectfont #1\par}
    \vspace{12pt}
}
...
\begin{titlepage}
    \thispagestyle{empty}
    \begin{physicalcenter}\sffamily
        {\bfseries\fontsize{15}{18}\selectfont MINISTRY OF EDUCATION AND TRAINING\par}
        {\bfseries\fontsize{15}{18}\selectfont FPT UNIVERSITY\par}
        \vspace{72pt}
        \begin{singlespace}
            {\fontsize{15}{18}\selectfont \thesistitletext\par}
        \end{singlespace}
        \vspace{71pt}
        {\fontsize{12}{14.4}\selectfont by\par}
        \vspace{78pt}
        {\fontsize{12}{14.4}\selectfont Tran Tien Duat\par}
        \vspace{88pt}
        {\fontsize{12}{14.4}\selectfont
        A thesis submitted in conformity with the requirements\\
        for the degree of Master of Software Engineering\par}
        \vspace{136pt}
        {\fontsize{12}{14.4}\selectfont \textcopyright{} Copyright by Tran Tien Duat 2026\par}
    \end{physicalcenter}
\end{titlepage}
...
\begin{titlepage}
    ...
    \begin{physicalcenter}\sffamily
        ...
        for the degree of Master of Software Engineering\par}
        \vspace{25pt}
        {\fontsize{12}{14.4}\selectfont Supervisor:\par}
        {\fontsize{12}{14.4}\selectfont Dr Doan Xuan Huy Minh\par}
        \vspace{76pt}
        {\fontsize{12}{14.4}\selectfont \textcopyright{} Copyright by Tran Tien Duat 2026\par}
    \end{physicalcenter}
\end{titlepage}
...
\vspace*{-26pt}
\begin{physicalcenter}\begin{singlespace}
    {\sffamily\fontsize{16}{22}\selectfont \thesistitledisplay\par}
    \vspace{16pt}
    {\fontsize{12}{14.4}\selectfont Tran Tien Duat\par}
    \vspace{10pt}
    {\fontsize{12}{14.4}\selectfont Degree Master of Software Engineering\par}
    \vspace{10pt}
    {\fontsize{12}{14.4}\selectfont FPT University\par}
    \vspace{10pt}
    {\fontsize{12}{14.4}\selectfont 2026\par}
\end{singlespace}
\end{physicalcenter}
\vspace{12pt}
\frontmatterheadingleft{Abstract}
...
This thesis designs and evaluates a \textbf{Hybrid-RAG} architecture that combines an \gls{ast}-derived \gls{kg} in FalkorDB with dense retrieval in Qdrant. The implementation uses query-aware graph traversal, vector search, weighted \gls{rrf} \cite{cormack2009rrf}, and token-budgeted context assembly. Paired benchmarks across pinned LlamaIndex Core, Transformers, and LangChain Core workloads contain 230 source-reviewed questions and 1,380 generated and scored answers. Hybrid-RAG produced a statistically positive Faithfulness difference on Transformers (+0.169, 95\% CI [0.043, 0.297]) and LangChain (+0.089 [0.017, 0.163]), while the LlamaIndex difference was inconclusive. Answer Correctness was statistically positive on LlamaIndex (+0.046 [0.002, 0.094]) and Transformers (+0.052 [0.002, 0.104]), but inconclusive on LangChain. No repository showed a conclusive Answer Relevancy difference. Hybrid retrieval added 182--237 ms of mean retrieval latency, while local generation remained the dominant cost at approximately 19--97 seconds. All embedding, generation, and judging calls used local Ollama services. The evidence supports replication across three Python repository workloads relative to vector-only retrieval, but not cross-language or alternative-baseline superiority.
...
I would like to express my sincere gratitude to my supervisor, Dr Doan Xuan Huy Minh, for the guidance, thoughtful feedback, and encouragement provided throughout this thesis. I also thank the lecturers and reviewers at FPT University for their comments and recommendations, which helped improve both the research and this report. Finally, I am grateful to my family, colleagues, and friends for their patience and support during the completion of this work.
...
\begin{itemize}
    ...
    \item \textbf{Evidence gap:} repository-specific point estimates are insufficient for broad claims without paired uncertainty estimates, replicated workloads, and stronger baselines.
\end{itemize}
...
\begin{itemize}
    ...
    \item reproducible paired answer-quality benchmarks across three pinned Python repository workloads, with raw records, case-level confidence intervals, and explicit limits on corpus, language, and baseline coverage.
\end{itemize}

\section{Research Objectives \& Questions}

\subsection{Research Objectives}
The primary objective is to determine whether adding AST-derived structural retrieval to an otherwise identical vector-RAG pipeline improves grounded answers across multiple Python repository workloads under local execution constraints. The specific objectives are:
\begin{itemize}
    ...
    \item compare Hybrid-RAG with vector-only retrieval using matched questions, pinned source identities, models, and repeated runs within each repository; and
...
\begin{itemize}
    \item \textbf{RQ1 --- Answer quality:} Across the pinned LlamaIndex, Transformers, and LangChain workloads, how does Hybrid-RAG change Faithfulness, Answer Relevancy, and Answer Correctness relative to vector-only retrieval?
...
The evaluated implementation is scoped to Python. Java indexing is explicitly rejected because a Java entity extractor and resolver are not implemented, even though a Java grammar is used by the separate context skeletonizer. The completed answer-quality evidence covers pinned LlamaIndex Core, Transformers, and LangChain Core workloads and one executed baseline (vector-only). It uses \texttt{nomic-embed-text} for embeddings, \texttt{gemma4:12b} for answer generation, and \texttt{qwen2.5-coder:7b} as an independent local judge. The LlamaIndex workload contains 30 questions; the Transformers and LangChain workloads contain 100 questions each. Their indexed corpus scopes are not identical, so results are reported per repository rather than pooled. Results for Kubernetes, BM25, graph-only retrieval, or Microsoft GraphRAG are not available and are not inferred.
...
\begin{enumerate}
    ...
    \item treat pooled cross-repository, cross-language, and additional-baseline conclusions as unverified until matched dedicated experiments are completed.
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item $\phi: V \rightarrow L_V$ is the node labeling function mapping vertices to $L_V = \{\text{Module}, \allowbreak \text{Class}, \allowbreak \text{Function}\}$.
        \item $\psi: E \rightarrow L_E$ is the edge labeling function mapping connections to syntax dependencies $L_E = \{\text{DEFINES}, \allowbreak \text{CALLS}, \allowbreak \text{INHERITS}, \allowbreak \text{IMPORTS}\}$.
    \end{itemize}
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Phase 4 --- Parallel Embedding:} Chunks are embedded through a bounded \texttt{ThreadPool\allowbreak Executor}; the default \texttt{EMBED\_\allowbreak CONCURRENCY} is 8. The \texttt{nomic-embed-text} model \cite{nussbaum2024nomic} generates 768-dimensional vectors through local Ollama \cite{ollama2024}. No controlled sequential-versus-parallel embedding benchmark is reported in this thesis.
\end{enumerate}
...
\begin{table}[H]
    ...
    \begin{tabular}{p{3.1cm}p{4.5cm}p{5.5cm}}
        ...
        \midrule
        \texttt{Repository\allowbreak Metadata} & \texttt{id, repository, last\_indexed\_commit} & Repository identity and synchronization state. \\
        ...
        \texttt{Document}/\allowbreak\texttt{Configuration} & \texttt{id, name, file\_path, repository, text} & Optional generic Markdown, YAML, and Dockerfile content. \\
...
RRF was selected instead of adding graph and vector scores directly because the two retrieval channels do not expose scores on a guaranteed common scale. Direct weighted addition would therefore require an additional normalization and calibration rule before the weights could be interpreted consistently. Rank fusion avoids that cross-channel scale assumption, while the configured weights still express the intended priority of each channel. A normalized weighted-sum fusion is a reasonable alternative, but it was not implemented as a controlled baseline in this thesis; the results therefore do not claim that RRF is universally superior.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        font=\small,
        level/.style={draw, rounded corners, fill=blue!10, minimum height=0.8cm, minimum width=3.8cm, align=center},
        level2/.style={draw, rounded corners, fill=purple!10, minimum height=0.8cm, minimum width=3.8cm, align=center},
        level3/.style={draw, rounded corners, fill=green!10, minimum height=0.8cm, minimum width=3.8cm, align=center},
        level4/.style={draw, rounded corners, fill=orange!10, minimum height=0.8cm, minimum width=3.8cm, align=center},
...
\begin{itemize}
    \item \textbf{Concurrent repository indexing:} API background jobs use an \texttt{asyncio.Semaphore} whose \texttt{INDEXING\_\allowbreak CONCURRENCY} default is 3.
    \item \textbf{Parallel AST parsing:} files within one repository are parsed with a bounded \texttt{ThreadPool\allowbreak Executor}; the worker limit is derived from CPU count and capped at 32.
\end{itemize}
...
The codebase is validated via 313 passing automated tests, with 27 explicitly skipped integration cases and two warnings in the final merged verification. The suite covers parsing and extraction logic, end-to-end ingestion pipelines, evaluation preflight, and functional API surfaces. The answer-quality benchmarks are separate controlled workloads totaling 1,380 generated and scored records and are not counted as unit-test coverage.
...
\begin{itemize}
    ...
    \item \textbf{Indexed sources:} pinned \texttt{llama-index-core==0.14.21} with deterministic source digest \texttt{sha256:\seqsplit{13b55bcf111885b7c2c9c07886b5656769041331c81c28bcf3cbfb87939ec509}}; Transformers commit \texttt{\seqsplit{0a2757da521a7a49b8143d9e0c938f08747d682e}}; and LangChain \texttt{libs/core} at commit \texttt{\seqsplit{51578289bb1f696a643e0740be1441039d8af8ce}}. All use index schema version 3 and \texttt{nomic-embed-text}.
    \item \textbf{Answer-quality suites:} LlamaIndex contains 30 cases (10 simple, 10 medium, 10 hard); Transformers and LangChain each contain 100 cases (35 simple, 35 medium, 30 hard). Every suite evaluates two retrieval modes with three repeats and seed 42, producing 1,380 generated and scored answers in total.
    \item \textbf{Statistical unit:} the question case within each repository; the three repeats are averaged within each case before paired bootstrap resampling with 10,000 resamples.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Hit Rate @K} (K=5): Measures the frequency of relevant documents appearing in the top-K returned results. It is retained as a diagnostic metric definition because a developer often needs at least one correct code block to proceed, rather than all K blocks being correct \cite{thakur2021beir}. However, no valid schema-v3 Hit Rate@5 result is used as confirmatory evidence in this thesis: the historical structural report was generated against schema-v2 and failed the current provenance check.
    ...
    \item \textbf{Paired 95\% Confidence Interval}: Quantifies uncertainty in Hybrid-minus-vector differences by resampling question-level pairs with replacement within each repository. Repeats are averaged within a case to avoid treating them as independent questions.
...
The repository also contains a historical 50-case structural report generated against a schema-v2 index. Its current status file explicitly records that it predates the schema-v3 identity repair and that its RAGAS field is null. Because it does not satisfy the present provenance preflight, this thesis does not use its Hit Rate values as current confirmatory evidence. The analysis below is restricted to the three completed schema-v3 answer-quality artifacts, whose source identity, index run, embedding model, raw records, and review grade are persisted. The Transformers index provenance identifies a controlled \texttt{gold-sparse} source tree, whereas LangChain identifies the full \texttt{libs/core} subtree; this corpus-scope difference precludes treating the repository estimates as one pooled experiment.

\section{Independent Answer-Quality Benchmark}
We separately measured generated-answer quality with the RAGAS metrics \cite{shahul2023ragas}. Each repository workload evaluates vector-only and Hybrid-RAG modes and repeats each case three times with seed 42. The answer model is local \texttt{gemma4:12b}; an independent local \texttt{qwen2.5-coder:7b} judge scores Faithfulness, Answer Relevancy, and Answer Correctness. LlamaIndex contributes 180 records, while Transformers and LangChain contribute 600 records each, for 1,380 generated and scored records.

The gold dataset is a manually maintained, checked-in artifact rather than a live dataset synchronized automatically with the latest repository release. Each case is reviewed against its pinned source snapshot; LlamaIndex records package/source digests, while the two Git-based workloads record immutable source commits and index provenance. A benchmark run is accepted only when these identities and the index provenance match; otherwise the run is rejected or treated as draft evidence. This makes the datasets auditable, but the manual maintenance process and AI source review remain limitations and do not replace independent human annotation.

\begin{table}[H]
    \centering
    \caption{Answer quality by repository with paired case-level 95\% confidence intervals}
    \label{tab:answer_quality_results}
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{llccc}
        \toprule
        \textbf{Repository} & \textbf{Metric} & \textbf{Vector mean [95\% CI]} & \textbf{Hybrid mean [95\% CI]} & \textbf{$\Delta$ [95\% CI]} \\
        \midrule
        LlamaIndex & Faithfulness & 0.896 [0.816, 0.956] & 0.924 [0.843, 0.980] & +0.029 [$-0.069$, 0.115] \\
        ...
        \midrule
        Transformers & Faithfulness & 0.441 [0.344, 0.538] & 0.610 [0.514, 0.702] & +0.169 [0.043, 0.297] \\
        & Answer Relevancy & 0.716 [0.656, 0.772] & 0.772 [0.719, 0.819] & +0.055 [$-0.014$, 0.126] \\
        & Answer Correctness & 0.574 [0.523, 0.625] & 0.626 [0.582, 0.669] & +0.052 [0.002, 0.104] \\
        \midrule
        LangChain & Faithfulness & 0.856 [0.786, 0.920] & 0.945 [0.900, 0.983] & +0.089 [0.017, 0.163] \\
        & Answer Relevancy & 0.854 [0.823, 0.877] & 0.845 [0.818, 0.866] & $-0.009$ [$-0.043$, 0.026] \\
        & Answer Correctness & 0.699 [0.662, 0.734] & 0.717 [0.679, 0.754] & +0.018 [$-0.021$, 0.057] \\
        \bottomrule
    \end{tabular}
    }
\end{table}

The paired intervals show a repository-dependent pattern. For LlamaIndex, only Answer Correctness excludes zero. For Transformers, Faithfulness and Answer Correctness exclude zero, while Answer Relevancy is inconclusive. For LangChain, only Faithfulness excludes zero; Answer Relevancy and Answer Correctness are inconclusive. Thus, Faithfulness replicates positively in the two new 100-case workloads, but no quality metric is conclusively positive in all three repositories. The source-hit rates are 1.00 in both LlamaIndex modes, 0.94 vector versus 0.97 hybrid for Transformers, and 1.00 vector versus 0.99 hybrid for LangChain. These source-hit fields are diagnostics and are not substituted for the excluded schema-v2 Hit Rate@5 result. The gold answers were approved by an AI source review, not independent human annotation; therefore the results are controlled local evidence rather than human-evaluated production accuracy rates.

\section{Generalizability and Baseline Coverage}
The requested replication on Transformers and LangChain is complete. Table~\ref{tab:evaluation_coverage} separates completed repository evidence from experiments that remain outstanding.
...
\begin{table}[H]
    ...
    \begin{tabular}{p{3.2cm}p{3.2cm}p{6.4cm}}
        ...
        Transformers & Completed & 100 reviewed questions, 3 repeats, pinned Git commit, and controlled \texttt{gold-sparse} index. \\
        LangChain & Completed & 100 reviewed questions, 3 repeats, pinned Git commit, and independent \texttt{libs/core} index. \\
...
Consequently, the benchmark answers RQ1--RQ3 separately for three pinned Python repository workloads. Replication across Transformers and LangChain is stronger evidence than a single-repository result, but the unequal case counts and corpus scopes do not support a pooled effect estimate. The experiment also does not establish generalization across programming languages or retrieval families.
...
The completed answer-quality benchmarks recorded retrieval, local generation, and token statistics for both retrieval modes. Table~\ref{tab:latency_breakdown} reports case-level means and paired-bootstrap intervals per repository; these are not service-level percentiles.

\begin{table}[H]
    \centering
    \caption{Latency and token breakdown by repository}
    \label{tab:latency_breakdown}
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{llccc}
        \toprule
        \textbf{Repository} & \textbf{Measure} & \textbf{Vector mean [95\% CI]} & \textbf{Hybrid mean [95\% CI]} & \textbf{$\Delta$ [95\% CI]} \\
        \midrule
        LlamaIndex & Retrieval (ms) & 134 [118, 151] & 371 [348, 395] & +237 [208, 266] \\
        & Generation (ms) & 96,754 [84,413, 110,142] & 87,268 [78,110, 96,465] & $-9,486$ [$-23,036$, 1,438] \\
        & Prompt tokens & 1,841 [1,633, 2,047] & 1,740 [1,566, 1,913] & $-101$ [$-282$, 77] \\
        & Completion tokens & 934 [788, 1,092] & 834 [734, 938] & $-100$ [$-259$, 30] \\
        \midrule
        Transformers & Retrieval (ms) & 138 [127, 149] & 319 [307, 331] & +182 [166, 197] \\
        & Generation (ms) & 47,892 [44,298, 51,487] & 44,005 [40,497, 47,487] & $-3,887$ [$-6,995$, $-938$] \\
        & Prompt tokens & 2,161 [2,031, 2,294] & 2,323 [2,169, 2,480] & +162 [$-6$, 328] \\
        & Completion tokens & 330 [289, 370] & 263 [224, 303] & $-67$ [$-102$, $-33$] \\
        \midrule
        LangChain & Retrieval (ms) & 105 [95, 116] & 316 [300, 332] & +211 [192, 230] \\
        & Generation (ms) & 21,407 [19,297, 23,557] & 19,499 [17,751, 21,302] & $-1,908$ [$-3,815$, $-27$] \\
        & Prompt tokens & 1,788 [1,599, 1,976] & 1,607 [1,442, 1,774] & $-181$ [$-380$, 17] \\
        & Completion tokens & 65 [53, 77] & 52 [44, 62] & $-12$ [$-24$, $-1$] \\
        \bottomrule
    \end{tabular}
    }
\end{table}

Hybrid retrieval adds a statistically positive mean retrieval overhead in every workload: 237 ms for LlamaIndex, 182 ms for Transformers, and 211 ms for LangChain. The LlamaIndex generation-latency and token intervals include zero. In contrast, generation latency and completion tokens are lower for Hybrid-RAG on both new 100-case workloads, although prompt-token differences remain inconclusive. Local generation dominates end-to-end latency for both modes; the results support a sub-second mean retrieval claim on this machine, not a sub-second answer-response SLA.

The end-to-end user experience includes retrieval, context assembly, prompt processing, and local answer generation. Consequently, the measured retrieval target and the total time before a developer receives an answer are separate quantities: sub-second retrieval does not imply interactive end-to-end response time when mean local generation ranges from approximately 19 to 97 seconds across the measured workloads.
...
\section{Conclusion}
The completed experiments support a replicated but repository-specific conclusion:
\begin{enumerate}
    \item \textbf{RQ1:} Hybrid-RAG has a statistically positive Faithfulness difference on Transformers and LangChain, and a statistically positive Answer Correctness difference on LlamaIndex and Transformers. Answer Relevancy is inconclusive in all three repositories.
    \item \textbf{RQ2:} Mean retrieval latency increases by 182--237 ms across the workloads. Local generation remains dominant at approximately 19--97 seconds; the two 100-case workloads show lower Hybrid-RAG generation latency and completion-token counts, while the LlamaIndex intervals remain inconclusive.
    \item \textbf{RQ3:} No quality metric has a positive paired 95\% interval in all three repositories. The replication strengthens the evidence beyond one repository but does not establish pooled cross-repository, cross-language, or alternative-baseline superiority.
\end{enumerate}
...
\begin{itemize}
    ...
    \item reproducible benchmarks totaling 1,380 generated and scored records across three pinned repository workloads, with deterministic paired case-level confidence-interval analyses.
\end{itemize}

\section{Limitations}
\label{sec:limitations}
\begin{itemize}
    \item \textbf{Local Generation Latency:} Mean answer generation ranges from approximately 19 to 87 seconds for hybrid retrieval and 21 to 97 seconds for vector-only retrieval across the measured workloads; this is not a sub-second end-to-end system.
    ...
    \item \textbf{Evaluation Scale:} The answer-quality evidence covers three pinned Python repository workloads with AI-reviewed reference answers, but LlamaIndex has 30 cases while Transformers and LangChain have 100 each. The corpus scopes also differ, so the estimates are reported separately and do not constitute a pooled benchmark. Independent human review is still required.
    ...
    \item \textbf{Statistical Power:} Significance varies by repository and metric, and no Answer Relevancy interval excludes zero. More independent questions and a matched cross-repository design are required before interpreting the estimates as universal improvements.
\end{itemize}

\section{Future Work}
\begin{itemize}
    \item \textbf{Standardized Multi-Repository Replication:} Expand LlamaIndex to 100 cases, apply the same corpus-selection policy to all repositories, add further Python repositories, and report a pooled estimate only after those design differences are removed.
    ...
    \item \textbf{Literature Refresh:} Add verified 2025--2026 references on code-RAG, repository-level retrieval, and agentic software engineering before the final report submission.
\end{itemize}
...
\sloppy
\begin{singlespace}
\titlespacing*{name=\chapter,numberless}{0pt}{-16pt}{18pt}
...
\clearpage
\phantomsection
\frontmatterheading{Copyright Acknowledgements}
Except where otherwise stated through citations and source acknowledgements, the text, diagrams, tables, and experimental analysis in this thesis were prepared by the author. Third-party publications, software, datasets, and online resources are identified in the References. Their inclusion is solely for academic discussion and does not transfer ownership of the original material.
