...
To solve this, we adapt **Microsoft's GraphRAG (Option A: Global Search)** architecture to an offline, privacy-preserving, local-first stack running on top of **FalkorDB**, **networkx**, and **Ollama (Qwen 2.5 Coder 14B)**.
...
2.  **Community Summarization (Offline Compilation):**
    ...
    *   Pass the community details to the local LLM (`qwen2.5-coder:14b`) with a specialized structural summary prompt.
...
*   **Synthesis (Map-Reduce / Direct Packing):**
    *   Since Qwen 2.5 Coder 14B supports a 128k context window, we can directly pack all community summaries (typically 3–8 communities, ~500 words each = ~4000 tokens) into the context window.
